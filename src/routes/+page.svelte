<script lang="ts">
    ...
    let savedCharacter: { name: string; stats: Record<string, number>; createdAt: string } | null = null;
    let createdParam = "";
    let name = "";
    ...
    function randomizeStats() {
      let total = 0;
      let generated: typeof baseStats;
      do {
        generated = {
      ...
      baseStats = generated;
      pointsRemaining = 5;
      addedStats = Object.keys(addedStats).reduce(
        (acc, key) => ({ ...acc, [key]: 0 }),
        {} as typeof addedStats
      );
    }
  
    function increaseStat(key: keyof typeof baseStats) {
    ...
    function decreaseStat(key: keyof typeof baseStats) {
      if (addedStats[key] > 0) {
        addedStats = { ...addedStats, [key]: addedStats[key] - 1 };
        pointsRemaining += 1;
      }
  ...
  <main>
    {#if characterCreated && savedCharacter}
      ...
      <button on:click={enterLife}>🌍 Enter World</button>
    {:else}
      <div>
        ...
        <form>
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button type="button" on:click={randomizeName}>🎲</button>
          </div>
          ...
          {#each Object.entries(baseStats) as [key, value]}
            <div class="stat-row">
              ...
              <button
                type="button"
                on:click={() => decreaseStat(key)}
                disabled={addedStats[key] === 0}
              >
                −1
              </button>
          ...
          <button class="randomize" type="button" on:click={randomizeStats}>Randomize</button>
          <button class="create" type="button" on:click={createCharacter}>Create Character</button>
        </form>
      </div>
    {/if}
  </main>
