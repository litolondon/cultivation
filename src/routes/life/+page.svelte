<script lang="ts">
    ...
    // Character with unallocated points pool
    type Character = {
      name: string;
    ...
    // Event pools with explicit weight percentages and good/bad flags
    const childhoodEvents: LifeEventDef[] = [
      ...
      { title: 'Nothing happens', description: 'Nothing special happened this year.', pointsReward: 0, optional: true, weight: 0.30, good: false }
    ];
    ...
    const chanceEvents: LifeEventDef[] = [
      ...
      { title: 'Nothing happens', description: 'Nothing unusual occurred.', pointsReward: 0, optional: true, weight: 0.30, good: false }
    ];
    ...
    const sectEvents: LifeEventDef[] = [
      ...
      { title: 'Nothing happens', description: 'Nothing of note happened in the sect.', pointsReward: 0, optional: true, weight: 0.30, good: false }
    ];
    ...
    // Advance one year and award spendable points
    function advanceYear() {
      if (!character) return;
      character.age += 1;
      const eventsThisYear: LifeEventDef[] = [];
  
      // Childhood: always 1 event per year
      if (character.age < 16) {
        const e = weightedPick(childhoodEvents);
        eventsThisYear.push(e);
      }
      ...
      if (character.age >= 16) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          // Apply and record each event, awarding points only for good events and accumulating Qi
      eventsThisYear.forEach(def => {
        // Accumulate Qi by pointsReward regardless of good/bad
        character!.qiPoints += def.pointsReward;
        // Award unallocatedPoints only for good events
        const reward = def.good ? def.pointsReward : 0;
        character!.unallocatedPoints += reward;
      ...
      saveCharacter();
      // Check for breakthrough after update
      tryBreakthrough();
    }
    ...
    onMount(() => {
      ...
      if (character.age === undefined) character.age = 0;
      if ((character as any).unallocatedPoints === undefined) character.unallocatedPoints = 0;
      if ((character as any).qiPoints === undefined) character.qiPoints = 0;
      if ((character as any).foundationReached === undefined) character.foundationReached = false;
